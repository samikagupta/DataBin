# from https://github.com/maxpumperla/entropy-mdlp
...
def cut_points(x, y, n):
    ...
    def getIndex(low, upp, depth=depth):
        ...
        # print(ret)
        if ret is not None:
            # print(cut_index, depth, x[cut_index])
            return [cut_index, depth + 1, ret] 
        else:
            return None
    ...
    # print(sorted(gain_val, reverse=True))
    # print(np.unique(res))
    # print(res)
    cut_index = None
    cut_value = []
    ...
    if res is not None:
            ...
            for indices in cut_index:
                    # print((xo[indices-1] + xo[indices])/2.0)
                    cut_value.append((xo[indices-1] + xo[indices])/2.0)
            for g in gain_val:
                if len(gain_bins) == n-1:
                    break
                gain_bins.append((xo[g[1]-1] + xo[g[1]])/2.0)
    # print(sorted(gain_bins))
    return sorted(gain_bins)
    # print(cut_value)
